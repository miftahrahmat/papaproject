...
<div class="px-4 py-3 z-50 sticky top-0 bg-sky-500 text-white">
    @if(Auth::user())
    <a href="{{ url('/akun/riwayat/transaksi') }}">
    ...
    <div class="relative h-[430px] w-full">
        ...
        <div class="absolute z-auto h-[200px] w-full bg-gradient-to-r from-[#10A8E5] via-[#53CAFA] to-[#53CAFA] px-[14px] pt-[33px] text-white">
            ...
            <div class="flex justify-between py-[8px] font-bold text-sm">
                <span>{{ \Carbon\Carbon::parse($exp)->translatedFormat('l, j F Y') }}</span>
                <div>
        ...
        <div class="absolute z-auto h-[200px] w-full bg-gradient-to-r from-[#10A8E5] via-[#53CAFA] to-[#53CAFA] px-[14px] pt-[33px] text-white">
            ...
            @elseif($data->status == "Proses")
                <span class="inline-block whitespace-nowrap rounded-full bg-yellow-100 px-[0.65em] pb-[0.25em] pt-[0.35em] text-center align-baseline text-[0.75em] font-bold leading-none text-warning-700">
                    Proses
                </span>
            @endif
        </div>
        ...
        <div class="absolute top-[130px] z-0 w-full rounded-[10px] bg-white px-[16px] py-[20px]">
            <div class="flex justify-between">
                <span class="pt-[8px] font-bold text-[#3a3a3a]">Pembayaran 
                    <span class="inline-block whitespace-nowrap rounded-full bg-warning-100 px-[0.65em] pb-[0.25em] pt-[0.35em] text-center align-baseline text-[0.75em] font-bold leading-none text-warning-700">
                        {{ $data->spay }}
                    </span>
                </span>
            ...
            <div class="mt-[15px] flex justify-between bg-[#f9f9f9] p-[20px]">
                <div class="text-[#4a4a4a]">
                    <span class="block uppercase">{{ $pay->pemilik }}</span>
                    <span class="block text-[16px] font-bold" id="nomor">{{ $data->nopay }}</span>
                </div>
            ...
            <div class="flex justify-between p-[20px] leading-6">
                <div>
                    <span class="block text-[12px] text-[#6A6A6A]">Nominal Pembayaran</span>
                    <span class="block text-lg font-bold" id="nominal">Rp{{ number_format($data->harga,0,',','.') }}</span>
                </div>
            ...
            <div class="relative top-[-30px]">
                ...
                <div class="flex justify-between rounded-[4px] border-[0.5px] border-[#FFBD20] bg-[#FFF8E8] py-5 px-3 text-sm">
                    ...
                    <div class="ml-[10px]">
                        ...
                        <span>Pastikan transfer tepat sampai 3 angka terakhir agar transaksi terverifikasi otomatis maksimal 1x24 jam.</span>
                    </div>
                </div>
    ...
    <div class="p-4 border-y-8 border-gray-100 mt-[4rem]">
        <div class="flex justify-between mb-2">
            <span class="font-bold text-[#3a3a3a]">Panduan Pembayaran</span>
            ...
            <div class="mt-[15px] p-[3px]">
                <ol class="my-[5px] mx-2 list-decimal text-sm font-normal">
                    ...
                    <li class="my-[5px] leading-[25px]">Isi 
                        ...
                        tujuan sesuai yang ada di halaman ini 
                        <span class="font-bold">(a.n {{ $pay->pemilik }})</span>.
                    </li>
                    <li class="my-[5px] leading-[25px]">Masukan 
                        <span class="font-bold">nominal pembayaran </span>
                        sesuai jumlah, termasuk <span class="font-bold">3 digit terakhir</span>
                    </li>
                    @if($pay->keterangan == "Manual")
                    <li class="my-[5px] leading-[25px]">Pembayaran akan diverifikasi oleh Papa Official Store. Waktu verikasi paling lambat 1x24 jam untuk metode pembayaran manual.</li>
                    @else
                    <li class="my-[5px] leading-[25px]">Pembayaran akan diverifikasi otomatis oleh robot Papa Official Store. Interval max < 20 menit </li>
                    @endif
                </ol>
            </div>
        </div>
        <div class="flex justify-between">
            <span class="font-bold text-[#3a3a3a]">Detail Transaksi</span>
            <div class="mt-[4px] cursor-pointer">
                <svg class="w-[18px] h-[24px] trx fill-[#949494]" viewBox="0 0 512 512" xmlns="http://www.w3.org/2000/svg">

                    <!--! Font Awesome Free 6.4.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2023 Fonticons, Inc. -->
                    <path d="M256 0a256 256 0 1 0 0 512A256 256 0 1 0 256 0zM135 241c-9.4-9.4-9.4-24.6 0-33.9s24.6-9.4 33.9 0l87 87 87-87c9.4-9.4 24.6-9.4 33.9 0s9.4 24.6 0 33.9L273 345c-9.4 9.4-24.6 9.4-33.9 0L135 241z"></path>
                  
                </svg>
                <svg class="w-[18px] h-[24px] trxup fill-[#949494]" viewBox="0 0 512 512" xmlns="http://www.w3.org/2000/svg">
        ...
        <div class="px-[4px] pesanan">
            <div class="mt-[15px] p-[3px]">
                <div class="flex justify-between mb-4">
                    <span class="font-bold text-sm">Layanan</span>
                    <span class="font-bold text-sm">{{ $data->layanan }}</span>
                </div>
                <div class="flex justify-between mb-4">
                    <span class="font-bold text-sm">Harga</span>
                    <span class="font-bold text-sm">Rp{{ number_format($data->harga,0,',','.') }}</span>
                </div>
                <div class="flex justify-between">
                    <span class="font-bold text-sm">ID/Tujuan</span>
                    <span class="font-bold text-sm">{{ $data->user_id }}
                        @if($data->zone) - {{ $data->zone }} @endif
                    </span>
                </div>
            </div>
        </div>
...
<script type="module">
    ...
    $(document).ready(function() {
      ...
      $(".trxup").hide();
      $(".trx").click(function(){
        $(".pesanan").slideDown('slow').addClass('animate__animated animate__fast animate__bounceInDown');
        $(".trx").hide();
        $(".trxup").slideDown('slow');
      }),
      $(".trxup").click(function(){
        $(".trx").slideDown('slow');
        $(".pesanan").hide('slow');
        $(".trxup").hide();
      })
